#!/local/packages/bin/python3
# -*- coding: utf-8 -*-

# ECE568: Lab 2, Part 2
# Biometric Mobile MFA

# Copyright 2021 by Courtney Gibson (gibson@ecf.utoronto.ca)
...
class BioConnect:
	...
	def getAuthenticatorStatus(self):
		...
		if result == False:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unable to get status of the mobile device")
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply status of the mobile device")
		...
		if ( status == "active" ):
			  if ( face_status == "enrolled" or voice_status == "enrolled" or fingerprint_status == "enrolled" or eye_status == "enrolled"):
				  return('active')
	...
	def sendStepup(self,
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply for POST")

		user_verification = reply.get("user_verification","")
		uuid = user_verification['uuid']
	...
	def getStepupStatus(self):
		...
		user_verif = reply.get("user_verification","")
		status = user_verif['status']
